...
function ChartGenerator(graphId, svgRef, canvasWidth, canvasHeight, gridSize = 20) {
    ...
    this.outer = d3_select(this.svgRef)
        .append('svg')
    ...
    this.vis = this.outer.append('g')
        ...
        .on('mousemove', function () {
            ...
            that.mouse_position = d3_touches(this)[0] || d3_mouse(this);
...
ChartGenerator.prototype.canvasMouseMove = function () {
    ...
    let i;
    this.mouse_position = [d3_event.pageX, d3_event.pageY];
...
ChartGenerator.prototype.resetMouseVars = function () {
    ...
    d3_select('.link_splice').classed('link_splice', false);
    if (this.spliceTimer) {
...
ChartGenerator.prototype.redrawNode = function () {
    const that = this;
...
ChartGenerator.prototype.redrawLink = function () {
    const that = this;
    ...
    link.attr('class', 'link').attr('class', 'link_background link_path').attr('d', function (_d) {
...
ChartGenerator.prototype.miniMapDraw = function () {
    ...
    d3_selectAll('.nodegroup').each(function (d) {

        miniMap.select('.contents').node().appendChild(this.cloneNode(true));

    });

    miniMap.select('.minimap').attr('viewBox', (d3_select('.contents').node().getBBox().x - 50) + ' ' +
        (d3_select('.contents').node().getBBox().y - 50) + ' ' +
        (d3_select('.contents').node().getBBox().width + 100) + ' ' +
        (d3_select('.contents').node().getBBox().height + 100)
    );

    contents.select('.viewfinder').attr('x', d3_select('.contents').node().getBBox().x)
        .attr('y', d3_select('.contents').node().getBBox().y)
        .attr('width', d3_select('.contents').node().getBBox().width)
        .attr('height', d3_select('.contents').node().getBBox().height);
...
ChartGenerator.prototype.portMouseUp = function (d, portType, portIndex) {
    ...
    if (this.mouse_mode === CANVAS_STATUS.JOINING || this.mouse_mode === CANVAS_STATUS.QUICK_JOINING) {
        if (typeof TouchEvent !== 'undefined' && d3_event instanceof TouchEvent) {
...
ChartGenerator.prototype.canvasMouseDown = function () {
    ...
    if (this.mouse_mode === CANVAS_STATUS.DEFAULT || this.mouse_mode === CANVAS_STATUS.QUICK_JOINING) {
        if (d3_event.metaKey || d3_event.ctrlKey) {
            point = d3_mouse(this);
            let ox = point[0];
            let oy = point[1];
    ...
    if (this.mouse_mode === CANVAS_STATUS.DEFAULT && !(d3_event.metaKey || d3_event.ctrlKey)) {
